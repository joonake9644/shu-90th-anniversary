...
export default function HistoryStory() {
  ...
  return (
    <div ref={containerRef} className="relative bg-black text-white w-full">
      {/* Scroll Height Spacer - Reduced to ~70% (350vh) */}
      <div className="h-[350vh] min-h-[200vh]"></div>

      {/* --- STICKY CANVAS --- */}
      <div className="fixed inset-0 w-full h-screen overflow-hidden flex items-center justify-center pointer-events-none">
        ...
        <motion.div style={{ opacity: prologueOpacity }} className="absolute inset-0 flex flex-col items-center justify-center z-50 bg-black">
            ...
            <motion.div 
                style={{ 
                    ...
                    boxShadow: useTransform(sparkGlow, (v) => `0 0 ${v} 4px rgba(251,191,36,0.6)`)
                }}
            ...
            <div className="absolute inset-0">
                {[...Array(50)].map((_, i) => (
                    <motion.div
                        ...
                        animate={{ opacity: Math.random(), scale: Math.random() * 1.5 }}
